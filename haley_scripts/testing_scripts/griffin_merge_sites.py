...
# merge_sites.py sometimes fails when running Griffin and given opaque results - here's a modified version of the script that can be run on its own. 
...
# Debug mode: Run one gene at a time instead of multiprocessing
for key, site_data in sites.items():
    print(f"Processing: {key}")  # Print gene name or key for debugging
    try:
        result = merge_sites([key, site_data])
        print(f"Success: {key}")
    except Exception as e:
        print(f"Error in {key}: {e}")
        break  # Stop on the first error to inspect
